...
export default function RetiroCuentaAhorros({ onVolver }: Props) {
  ...
  const generarNumeroTransaccion = () => {
    ...
    const random = Math.floor(Math.random() * 1000)
      .toString()
      .padStart(3, "0")
    return `TXC${timestamp.slice(-6)}${random}`
  }
  ...
  const handleValidarNumero = () => {
    if (!validarNumeroCuenta(numeroCuenta)) {
      setError("El número de cuenta debe tener exactamente 11 dígitos numéricos")
      return
    }
    setError("")
    setPaso("clave")
        ...
        <Card className="w-full max-w-md from-white to-white bg-gradient-to-br">
          <CardHeader>
            <div className="flex items-center space-x-2 ">
              ...
              <CardTitle className="text-black-900">Cuenta de Ahorros</CardTitle>
            </div>
          </CardHeader>
        ...
        <Card className="w-full max-w-md from-white to-white bg-gradient-to-br">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex space-x-3">
              <Button onClick={() => setPaso("clave-confirmacion")} className="flex-1 bg-blue-500 hover:bg-blue-700 text-white">
                Confirmar
              </Button>
